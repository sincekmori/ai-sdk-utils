...
| [`ai-sdk-catalog.advanced.json`](ai-sdk-catalog.advanced.json) | Everything above plus an `openai-compatible` vendor (Fireworks), your own gateway (multiple backends, `actionMap`, `slug` overrides), and per-model `providerOptions` merging. |
...
One provider kind is deliberately absent from these files: a **resolver-backed** provider (Amazon Bedrock, Google Vertex, Azure) is listed in the config like any other but wired in code via `createCatalog(config, { resolvers })` — with one here, `createCatalog` would throw unless you pass that resolver. See "Custom resolvers" in the [package README](../README.md).
