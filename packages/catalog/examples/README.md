...
| [`ai-sdk-catalog.standard.json`](ai-sdk-catalog.standard.json) | Several direct vendors, inherited default call settings, a per-model price sheet (`cost`), and a local OpenAI-compatible server (Ollama) — still config-only.                                                                 |
| [`ai-sdk-catalog.advanced.json`](ai-sdk-catalog.advanced.json) | Everything above plus an `openai-compatible` vendor (Fireworks), your own gateway (multiple backends including two of one vendor, `headers`/`query`, `actionMap`, `slug` overrides), and per-model `providerOptions` merging. |
